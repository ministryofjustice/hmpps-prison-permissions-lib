...
import { isInUsersCaseLoad, isReleased, userHasRole, userHasSomeRolesFrom, wasReleasedWithinThreeYears } from '../../../utils/PermissionUtils'
...
const contactsReadCheck = matchBaseCheckAnd({
  overridingCondition: (user, prisoner) => {
    ...
    if (!userHasSomeRolesFrom([Role.ContactsAdministrator, Role.ContactsAuthoriser], user)) return PermissionCheckStatus.ROLE_NOT_PRESENT
